...
from app.routers.products import _packaged_by_product, _sold_by_product
...
def create_sale(body: SaleRequest, user: dict = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    if product.recipe_id:
        # Блокируем строку продукта на время проверки+записи — без этого два одновременных
        ...
        # на code-review 2026-07-21). Раньше лочили Recipe как прокси-мьютекс — с переходом
        # на упаковку (2026-07-23) готовность больше не завязана на рецепт, лочим сам Product.
        db.execute(select(Product.id).where(Product.id == product.id).with_for_update())
        ready = _packaged_by_product(db, user["company_id"]).get(product.id, 0.0) - _sold_by_product(
            db, user["company_id"]
        ).get(product.id, 0.0)
        if body.qty > ready:
...
def update_sale(
    ...
    if product.recipe_id and (target_product_id != sale.product_id or target_qty != float(sale.qty)):
        # Та же блокировка строки продукта, что и в create_sale — см. комментарий там.
        db.execute(select(Product.id).where(Product.id == product.id).with_for_update())
        ...
        ready = _packaged_by_product(db, user["company_id"]).get(product.id, 0.0) - sold_excluding_self.get(
