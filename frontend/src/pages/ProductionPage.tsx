        ...
        {log.map((entry) => (
          <div key={entry.id} className="rounded-xl border border-ink/10 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-2">
              ...
              <span className="shrink-0 text-sm font-semibold text-ink">{entry['кол-во продукта']}</span>
            </div>
          ...
          <tbody>
            ...
            {log.map((entry) => (
              <tr key={entry.id} className="border-b border-ink/5 last:border-0">
                ...
                <td className="px-4 py-3 text-right font-medium">{entry['кол-во продукта']}</td>
